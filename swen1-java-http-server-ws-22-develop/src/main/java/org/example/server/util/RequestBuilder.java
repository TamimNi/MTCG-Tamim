...
public class RequestBuilder {
    ...
    private static String getContent(String requestString) throws UnsupportedProtocolException {
        String content = HttpRegex.findContent(requestString);

        if (null == content) {
            throw new UnsupportedProtocolException("No HTTP content in request");
        }

        return content;
    }
    //work here
    private static String getAuthorization(String requestString) {
        String authorization = HttpRegex.findHeaderAsString(requestString,"Authorization");
